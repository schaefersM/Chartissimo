...
router
    ...
    .post(async (req, res) => {
        const { user_id } = req.params;
        const { name, id, data, hosts, chartType, customOptions, image } = req.body;
        try {
...
router
    ...
    .get(async (req, res) => {
        const { user_id } = req.params;
        try {
            const chartNames = await UserChart.find({user: user_id}, {"name": 1, "_id": 0})
            res.data = {
                chartNames
            }
            return res.json(res.data);
...
router.route("/:id")
    ...
    .put(async (req, res) => {
        const { user_id, id } = req.params;
        const { name, image, customOptions, tableNames, customTableNames, colorIds } = req.body
        try {
            const checkForUpdateName = await UserChart.findOne({ user: user_id, name, id});
            if (checkForUpdateName) {
                await UserChart.updateOne({
                    ...
                    name,
                    image,
                    $set : { "data.tableNames" : tableNames , "data.customTableNames" : customTableNames, "data.colorIds": colorIds},
                    customOptions,
                    lastModified: new Date()
                })
                console.log(`Found chart and updated chart ${name} of user ${user_id}`)
            } else {
                const checkName = await UserChart.findOne({ user: user_id, name});
                if (checkName) {
